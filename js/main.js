...
(function(){
  ...
  s.textContent =
    ...
    '#galleryDesktopGrid > a[href="/galeri"]:hover{transform:scale(1.03);box-shadow:0 25px 50px -12px rgba(8,145,178,.4);background:linear-gradient(to top left,#0891B2,#22D3EE)!important}' +
...
if (nav && topBar) {
    ...
    function updateNav() {
        ...
        if (!hasHero || window.scrollY > topBarH) {
            nav.style.top = '0';
            nav.style.background = 'linear-gradient(135deg, rgba(6,78,92,0.97), rgba(8,145,178,0.95))';
            nav.style.backdropFilter = 'blur(20px)';
            nav.style.borderBottom = '1px solid rgba(16,185,129,0.15)';
...
(function(){
  ...
  fetch('/api/services.php').then(function(r){ return r.ok ? r.json() : null; }).then(function(d){
    ...
    d.services.forEach(function(svc){
      ...
      if (svc.items && svc.items.length) {
        ...
        itemsHtml = '<div style="margin-top:1.25rem;padding-top:1.25rem;border-top:1px solid rgba(8,145,178,0.08)">'
          ...
          + '<span style="display:inline-block;width:3px;height:14px;border-radius:2px;background:linear-gradient(180deg,#0891B2,#22D3EE)"></span>'
          + '<span style="font-size:0.7rem;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:#0891B2;opacity:0.7">Fiyat Detayları</span>'
          + '</div>'
          + '<svg class="acc-arrow" style="width:16px;height:16px;color:#0891B2;opacity:0.5;transition:transform 0.3s ease;flex-shrink:0" viewBox="0 0 24 24" fill="currentColor"><path d="M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6z"/></svg>'
          + '</button>'
        ...
        svc.items.forEach(function(item, idx) {
          ...
          itemsHtml += '<div style="display:flex;align-items:center;justify-content:space-between;padding:0.5rem 0;' + (isLast ? '' : 'border-bottom:1px solid rgba(8,145,178,0.04);') + '">'
            + '<div style="flex:1;min-width:0">'
            + '<span style="font-size:0.8rem;font-weight:500;color:rgba(8,145,178,0.85)">' + item.name + '</span>'
            + (item.description ? '<span style="display:block;font-size:0.7rem;color:#22D3EE;opacity:0.55;margin-top:1px">' + item.description + '</span>' : '')
            + '</div>'
            + '<div style="display:flex;flex-direction:column;align-items:flex-end;margin-left:1rem;white-space:nowrap">'
            + '<span style="font-size:0.85rem;font-weight:700;color:#0891B2;line-height:1.2">' + priceDisplay + '</span>'
            + (itemUnit ? '<span style="font-size:0.65rem;color:#22D3EE;opacity:0.7;margin-top:1px">' + itemUnit + '</span>' : '')
            + '</div>'
            + '</div>';
        });
        itemsHtml += '</div></div>';
      ...
      html += '<div class="group bg-surface-container-lowest rounded-xl border border-transparent hover:border-outline-variant/20 transition-all duration-500 hover:-translate-y-2 hover:shadow-xl hover:shadow-primary/5 flex flex-col" style="overflow:hidden">'
        ...
        + itemsHtml
        + '</div>'
        + '<a href="' + svcLink + '" style="display:flex;align-items:center;justify-content:space-between;padding:1rem 1.5rem;background:linear-gradient(135deg,rgba(8,145,178,0.03),rgba(34,211,238,0.06));border-top:1px solid rgba(8,145,178,0.08);text-decoration:none;transition:background 0.3s ease" onmouseenter="this.style.background=\'linear-gradient(135deg,rgba(8,145,178,0.07),rgba(34,211,238,0.12))\'" onmouseleave="this.style.background=\'linear-gradient(135deg,rgba(8,145,178,0.03),rgba(34,211,238,0.06))\'">'
        + (priceHtml || '<span></span>')
        + '<span style="font-size:0.8rem;font-weight:600;color:#0891B2;display:flex;align-items:center;gap:0.35rem;letter-spacing:0.02em">Detaylar <svg style="width:1.1em;height:1.1em;transition:transform 0.3s ease" viewBox="0 0 24 24" fill="currentColor"><path d="M12 4l-1.41 1.41L16.17 11H4v2h12.17l-5.58 5.59L12 20l8-8z"/></svg></span>'
        + '</a></div>';
    });
  ...
  fetch('/api/gallery.php').then(function(r){ return r.ok ? r.json() : null; }).then(function(d){
    ...
    if(desktopGrid){
      ...
      // 9. hücre: buton
      html += '<a href="/galeri" class="aspect-[4/3] rounded-2xl flex flex-col items-center justify-center gap-3 shadow-lg cursor-pointer reveal" style="background:linear-gradient(to bottom right,#0891B2,#22D3EE)">' +
